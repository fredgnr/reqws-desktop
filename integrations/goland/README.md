...
- [Compatibility development record](../../docs/changes/ide-plugin-compatibility-automation/implementation-2026-09-21.md), [platform/API verification](../../docs/changes/ide-plugin-compatibility-automation/verification-2026-09-21.md) and [local IDE regression](../../docs/changes/ide-plugin-compatibility-automation/local-verification-2026-09-22.md) distinguish implementation from executed evidence and remaining V acceptance.
...
To use an existing **GO 2026.2** compilation SDK, pass `-PreqwsGoLandSdkPath=/absolute/path/GoLand.app` to Gradle, or set `ORG_GRADLE_PROJECT_reqwsGoLandSdkPath`. Structured ProductInfo checks reject the wrong product, release or Java requirement. This option does not install or start that IDE and never changes the descriptor floor. `python3 scripts/run_ide_verifier.py --baseline --archive /absolute/path/candidate.zip --version 0.1.7 --output /fresh/output` retains the compilation and fixed representative API targets; it is not the complete candidate gate. Offline diagnostic verification retains API/dependency checks but does not replace the fresh online CI/release matrix.

The only approved restricted API is the initial-JPS wrapper's `WorkspaceModelInternal.awaitSynchronizationWithJpsModel` call. Its Internal type and Experimental method are checked at the exact source and bytecode call site. All Gradle failure levels remain enabled: raw `verifyPlugin` reports and fails on this use. The canonical runner accepts only the exact reviewed reports and matching candidate bytecode, preserves the nonzero raw exit and reports, and rejects any additional usage or failure. Use `check:goland` for the final result; see the [S4 exception record](../../docs/changes/playwright-regression-automation/implementation-s4-2026-09-26.md#6-已授权-api-例外).
...
npm run check:goland:desktop -- --profile "$HOME/.reqws-ide-tests/goland-2026.2.1.1" --archive /absolute/path/candidate.zip --version 0.1.7
```

`check:goland:desktop` selects the separate S4 suite: actual Electron UI writes drive the live IDE watcher, Project tree/model/PFI assertions, cold processes, trust dialogs and invalid-input recovery. It uses the same locked dedicated profile and exact plugin ZIP; it builds Desktop source only. Reports distinguish `suite=desktop` from the original `legacy` suite. Compilation, a Desktop-only protocol check and legacy UI evidence do not prove S4 passed; see the [S4 implementation record](../../docs/changes/playwright-regression-automation/implementation-s4-2026-09-26.md). Full IDE execution remains local and requires authorization for that dedicated environment.

Preparation opens a dedicated GoLand 2026.2.1.1 without a business project, where the user may sign in through JetBrains Account and then quit normally. GoLand may generate its own welcome workspace under the dedicated config; the next run isolates that directory too. Startup flags do not claim that the user has accepted agreements. `JETBRAINS_LICENSE_SERVER` is optional. The dedicated config is reused in place; personal licenses/tokens/settings are never copied or assumed inherited. Every run gets fresh business fixtures, project state, system/plugins/log directories and a profile lock. The Starter host uses an isolated JVM user home, and the global process-cleanup JUnit extension is excluded; only this run’s process handles may be cleaned up. The launcher never rebuilds the production plugin. Reports bind the actual ZIP, plugin version, IDE version/build and results; a signed artifact needs its own exact-byte evidence. Missing authorization, graphics or permissions blocks the run; preparation/CI success is not a UI pass. See [local integration instructions](../../docs/changes/ide-plugin-compatibility-automation/local-integration.md). See the [verification record](../../docs/changes/ide-plugin-compatibility-automation/verification-2026-09-21.md) for actual checks and the remaining acceptance scope. Packaging disables searchable-option indexing because ReqWS has no Settings configurables; it must not implicitly launch an IDE.
