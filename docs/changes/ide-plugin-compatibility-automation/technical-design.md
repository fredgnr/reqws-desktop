...
API 矩阵不设置 `max-parallel`，由 GitHub 根据可用 runner 和账户并发配额调度；容量不足时排队或明确报资源阻塞，不静默截断必需目标。

每周 main 与已发布候选的 API 矩阵仍按阶段执行，避免两组矩阵同时争用 runner；每组内部不另设并发上限。main 的失败不阻止已发布候选独立取证，取消的工作流不再启动后一组。
...
job 分为：影响与目标解析、基线测试/单次构建、API 分片及必需检查汇总，不包含 Starter/Driver job。影响分类中的本机集成建议只作提示，不触发用户电脑。API 每次冻结目标，并发由 GitHub 按可用 runner 和账户配额调度；保留逐目标终态及同一 ZIP 检查。CI 证据明确写 `scope=ci-api`、`localIntegration.status=not-run`；发布汇总不读取或伪造本机 UI 通过。更高版本仅扩展 API 集合，不增加多版本本机 GUI。
