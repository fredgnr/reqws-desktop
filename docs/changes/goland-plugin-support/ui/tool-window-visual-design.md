...
- 保留 `Sync Now`、`Open Manifest File` 和 `Copy Diagnostics` 的现有行为、资源键和安全边界，不新增业务入口。
...
原型由内置 ImageGen 的 `ui-mockup` 模式生成，并以旧版真实 GoLand 截图作为结构参考。提示词要求只优化右侧 ReqWS Tool Window，使用原生浅色 JetBrains 密度，形成状态标题、工作区摘要、三行仓库列表、摘要和分层操作区；禁止渐变、玻璃拟态、图表、装饰插画和额外操作。生成图中的 GoLand 外壳仅作视觉上下文，文字或项目树细节不作为产品事实。
...
| 仓库列表 | 使用独立卡片；header 左侧为标题、右侧为纯数字 count；每行固定紧凑高度并带主题分隔线，包含 repository name、文字状态和辅助色图标；1–6 行不显示滚动条，7 行起固定显示六行并在卡片内滚动。 |
| 诊断摘要 | 显示 digest、保留上次有效模型或无 manifest 提示；错误码继续与状态文字同屏。 |
| 操作区 | `Sync Now` 为全宽主操作；另两个动作居中显示为次级链接，并保持原有 enable 规则。 |
...
| degraded、Safe Mode | 警示色辅助标记 | 部分可用或信任提示 |
...
这张图片是“实现候选，待验收”，不表示 `PASS` 或 `GO`。它只证明当前候选已在真实日常 GoLand 中加载并呈现 synchronized 状态；尚未覆盖深色主题、错误、降级、Safe Mode、最窄宽度、键盘焦点和关闭项目/退出 IDE 生命周期。正式结论仍须由新的 dated verification 绑定 exact candidate 与 ZIP 哈希后给出；既有 `2026-08-17` `NO-GO` 报告保持为历史证据，不因本图而改写。
