...
| 文档 | 类型 | 状态 | 说明 |
|---|---|---|---|
| [需求说明](requirements.md) | requirements | active | 维护通用支持契约，并在完成定义、场景与矩阵中落实语言无关成功条件。 |
| [技术方案](technical-design.md) | technical-design | active | 维护模型/PFI、安全、恢复和 VCS 只读设计；标明已删除的 Go 门禁与补偿的历史入口。 |
...
[GoLand 工作加载集合](../goland-workspace-loading/README.md)按用户最新决定，不考虑任何兼容性：唯一 shell 入口、root 级所有权、两层适配直接替换此处的原目录实现；旧 adapter、状态迁移、回退入口和旧版本矩阵可删除。这里只记录替换前实现，不要求新方案保留它。提案仍是 draft，产品代码尚未改动；API 使用政策与无兼容要求分开处理。
