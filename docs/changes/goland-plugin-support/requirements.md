...
S1/S2 已实施，当前结果见 [2026-09-19 验收记录](../ide-plugin-language-decoupling/testing/acceptance-2026-09-19.md)。原 Go registry、`go.mod` 和 PACKAGE run/test 成功条件已由[语言解耦契约 LD-01～LD-06](../ide-plugin-language-decoupling/technical-design.md#2-目标与非目标)替代；下文完成定义、场景和矩阵已同步。原条款仅见[固定基线的历史需求](https://github.com/fredgnr/reqws-desktop/blob/409b30e573d47348618620bbc0a52c0dd0710954/docs/changes/goland-plugin-support/requirements.md)，不作为当前实现或验收要求。通用 manifest、ownership、PFI、信任、取消恢复与 VCS 只读要求继续有效。
...
3. **GoLand 插件 v0.1**：识别 ReqWS workspace，自动同步活动仓库的项目内容，只读检查 Git Root，监听 manifest、外部 Project Model drift 与 VCS 配置变化，并提供状态和手动配置诊断。

成功标准不是让 GoLand 支持 VS Code 的 workspace 文件，而是：

> 同一个 ReqWS workspace 在 GoLand 中表现为一个受管多仓库项目；authoritative Workspace Model 与 public `ProjectFileIndex` 自动向 manifest 收敛，Git Root 差异可见并由用户在 GoLand 中明确配置。
...
- 活动仓库对应的 authoritative Workspace Model 与 public `ProjectFileIndex` 均与 manifest 一致，Git Root 缺失或冲突有明确的手动配置步骤；
- 已逻辑移除但仍在磁盘上的仓库自动退出默认索引、搜索和代码分析；若仍是 Git Root，插件提示用户在 Directory Mappings 中手动移除；
- 仓库是否活动由 manifest 决定，不取决于 `go.mod` 或其他构建文件；`Synced`/`Active` 不承诺语言分析、SDK、依赖或运行/调试配置就绪；
...
- 把 authoritative Workspace Model 作为事实层，以 public `ProjectFileIndex` 验证活动/排除目录边界；模型或 PFI 不一致时不得推进 clean digest 或显示 `Synced`/`Active`；
- Go registry 读取、语言专属 roots 通知、等待与补偿已删除；不得通过可选适配器或恒真实现恢复旧成功门禁，详见[当前执行链](../ide-plugin-language-decoupling/technical-design.md#4-目标执行链与状态)；
- 不调用 Go API、tracker/scheduler、反射或 private API，也不启动 Go command 或下载；正常 Workspace Model 更新及其必要平台通知保留，GoLand 原生进程/下载与 ReqWS 调用分别归因；
- 订阅 public `ModuleRootListener` 识别用户、GoLand 或其他插件造成的 live project-model drift；外部事件以固定 250 ms 防抖并用 `PROJECT_MODEL_CHANGE` 强制 same-digest reconcile，ReqWS 自己在 mutation guard 内产生的同步事件必须抑制，未被 guard 抑制的迟到普通事件同样进入有界防抖的 `PROJECT_MODEL_CHANGE`，不得恢复 Go 专属 follow-up 或形成循环刷新；
...
- Safe Mode 下只允许读取和诊断，不修改项目模型、不发布 roots change event，也不启动外部进程；VCS 在 trusted 与 Safe Mode 下都保持只读。
...
以下保留原支持交付涉及的检查层；后续候选按[开发与测试规范](../../standards/ide-plugin-development-testing.md)和[当前测试计划](../ide-plugin-language-decoupling/testing/test-plan.md)选择受影响检查，不机械重跑 Desktop、原生 Git 或 Go 工具链全矩阵。
...
- 验证报告记录源码 commit/diff、GoLand version/build、JBR/JDK、macOS、工件入口、fixture 和结果；临时工件比对留在测试输出，不抄写摘要台账；
...
5. 插件检测 manifest，并在项目已受信时同步 authoritative Workspace Model、通过 public `ProjectFileIndex` 目录边界验证、只读检查 Git Roots；
...
- 对从未生成 `.idea` 的全新 workspace，GoLand 项目 metadata 尚未落盘属于启动期 readiness，不是 Project Model ownership 冲突。插件保持 `SYNCHRONIZING`，等待 GoLand 自己创建该目录后自动重读最新 manifest；不要求改写 manifest、点击 `Sync Now` 或重开项目；
- 若首次 read/apply 被一次瞬时取消，启动流程自身在 owner/service 仍存活且该取消仍为 latest 时自动重试并最终收敛，不要求用户修改 manifest 或重新打开项目；
- Safe Mode 时只读显示 workspace、当前 VCS 配置诊断和信任提示，不修改 Project Model/VCS，也不发布 ReqWS roots change event；
...
- 插件观察到最新有效 manifest；
- 新仓库进入 authoritative Workspace Model 并通过 public `ProjectFileIndex` 目录边界验证；不检查 Go module 或运行配置。若缺少精确 Git mapping，Tool Window 提示用户在 Settings → Version Control → Directory Mappings 中添加为 `Git`；
...
- 该目录不再参与默认受管项目内容和搜索范围；若它仍是 Git Root，Tool Window 提示用户按自身意图手动移除；
- 已打开文件可以由 GoLand 按平台行为保留为外部文件，但不得因此重新成为受管 root。

### 5.5 重新添加与恢复

- 重新添加保留仓库时，插件恢复同一路径的受管项目模型条目，验证 public `ProjectFileIndex` 并重新检查 Git mapping；不承诺语言运行配置即时恢复，也不把移除目录解释为禁止运行其磁盘代码；
...
- 首次 Project Model apply 遇到尚未创建的 `.idea` 时，只由单次 production startup lineage 启动 1 秒一次、最长 10 分钟的 exact-path readiness 等待；截止前发现任意 entry 后只提交一次 latest-manifest successor，由完整安全门禁决定成功或真实冲突。共享截止时间到达后，即使调度、wait 或 probe 曾被暂停，也不再追加等待、probe 或恢复 successor；仍有效的等待直接进入 `ERROR / PROJECT_MODEL_APPLY_FAILED`，不重跑旧 apply，并保留 `Sync Now`；
- 重复同步不产生重复条目；
...
插件也不得直接调用 Go module tracker/status tracker/scheduler、下载器或 process API。trusted project 中正常 Workspace Model 更新产生的平台通知可能触发 GoLand 自己的语言处理或依赖下载，必须在日志和验收中单独归因；旧 Go mismatch 专属 roots 通知已删除，不得描述为 ReqWS 直接执行命令。GoLand 用户主动使用 IDE 自带 Git 功能同样不属于插件自动行为。插件只负责检测当前 Directory Mappings 是否包含活动仓库的精确 Git Root，并显示如何手动修正，不负责登记或移除。
...
- Safe Mode 下无项目模型、ReqWS roots change event 和外部进程副作用；
...
- 同一 live project service/coordinator 生命周期内，只有 authoritative Workspace Model 与 public `ProjectFileIndex` 验证通过并建立 clean baseline 后，相同 manifest digest 才可跳过普通自动项目模型重放；VCS 继续独立只读诊断，其配置事件仍必须触发复核；
- `ProjectFileIndex` 未收敛时保持 baseline dirty 并发布稳定 `PROJECT_CONTENT_NOT_CONVERGED`，后续自动或手动 same-digest 请求仍可恢复；不得静默降级为 clean；
- public `ModuleRootListener` 的外部变化 burst 必须在 250 ms 内合并为一个 `PROJECT_MODEL_CHANGE` 强制 reconcile；mutation guard 抑制 ReqWS 自己的同步事件，未被 guard 抑制的迟到普通 event 仍经有界防抖提交 `PROJECT_MODEL_CHANGE`，不能形成 roots/apply 热循环；
...
- 插件不得创建、删除、清空或修复 `.idea`。只有 production startup lineage、cold model snapshot、pristine PSC mirror、从 service 启动以来从未观察到真实 `.idea`，且 exact `<root>/.idea` 的首次 `NOFOLLOW_LINKS` attribute read 抛出 `NoSuchFileException` 时，才可内部分类为 metadata readiness。`.idea` 为文件或 symlink、权限/identity 异常、曾观察后消失、既有 ownership proof、stable handle/lock/state/temp/readback 任一失败都继续 fail closed；readiness probe 只负责唤醒，不能成为写入授权。较旧 read 在开始后被较新 generation 淘汰，不得再取消较新 generation 的 readiness monitor；替换恢复任务与接受最新 read 必须保持一致。
- latest manifest/VCS read 收到 `ProcessCanceledException` 或 coroutine cancellation 时，仍必须原样结束该 read；若 request 仍为 latest 且 project/service 存活，UI 必须恢复进入 `READING` 前最近的稳定状态且不写业务 `lastError`。已有可见稳定状态时保留用户可达的 `Sync Now`；首次启动回滚到无 snapshot 的 `INACTIVE` 时，由 service scope 延迟提交一次有界 automatic successor，并以 predecessor generation 与 exact state version 防止旧重试覆盖更新 read/apply/dispose。旧 cancellation completion 晚到时也不得注册重试去替换较新 generation 的恢复任务，不能因此让首次启动永久停在 `INACTIVE`。
- 单次 Project Model applier 收到平台或 coroutine cancellation 时不得发布普通 `Failed`、不得仅因该 submission 把 coordinator 永久关闭，也不得保留 `SYNCHRONIZING`；该 submission 保持 dirty 并以同一终止信号结束，owner scope 仍 active 时后续 refresh 必须能在同一 service/coordinator 生命周期重新 apply。首次 apply 回滚到无 snapshot 的 `INACTIVE` 时复用同一项一次性自动恢复；retry 自身再次取消不得续订，避免 cancellation storm。owner scope 取消或 service dispose 仍永久停止 coordinator；observer 自身抛出的终止信号继续按既有 raw-propagation 边界结束 worker，包括只在一次 `Applying` 通知中抛出的 PCE/CE，不能被转为可恢复的 applier `Cancelled`。
...
- listener 回调不做阻塞 IO；
- 无目标变化时不触发重复索引；
- clean 状态下不允许周期性 roots event；clean same-digest 自动复核不重放项目模型，manual/trust/model-change intent 仍可强制复核，外部 roots burst 只产生一次防抖后的强制 reconcile；
...
- 初始 project-metadata readiness monitor 必须是 lifecycle-owned single instance；更新 generation 可以继承 startup lineage 以使用最新 manifest，但必须继承同一个 absolute deadline 和已消费 attempt。probe 每 tick 只读取 exact `.idea` attributes，不重复 manifest/project-model apply；timeout、success、hard failure、owner cancellation 或 dispose 后不得继续 churn 或晚到发布；
...
- 生产代码不依赖或调用 Go API；仅保留 GoLand 产品限制及实际使用的平台/VCS 依赖，当前依赖边界见[语言解耦方案](../ide-plugin-language-decoupling/technical-design.md#7-依赖与构建门禁)；
...
本表保留通用需求编号并移除已失效的语言条件。当前执行范围与必要 GUI 链以[最终验收计划](../ide-plugin-language-decoupling/testing/final-acceptance.md)为准；规模、视觉、Desktop 及其他风险专项按影响选择，不把历史 GO 当作新候选证据。
...
| GL-04 | 首次打开 | 插件识别 manifest，并按 authoritative Workspace Model → public `ProjectFileIndex` 顺序验证目录边界，不以构建文件或语言配置为成功条件；virgin root 即使启动时尚无 `.idea`，也由单次 production startup trigger 在 GoLand 创建 metadata 后自动收敛，不误报 `OWNERSHIP_CONFLICT`，不依赖 direct service API、manifest 改写、`Sync Now` 或 reopen；首次 read/apply 的一次 PCE 或 coroutine cancellation 同样由启动链路自动恢复；缺失 Git Root 有明确 Directory Mappings 手动步骤，用户配置后事件自动复核为已配置。 |
| GL-05 | 新增仓库 | Desktop 操作完成后 GoLand 无需重启即可加入受管项目内容和搜索范围，通过 Workspace Model/PFI 验证；缺失 Git Root 显示待用户配置且插件零 VCS 写入。 |
| GL-06 | 逻辑移除 | 磁盘目录保留并退出受管项目内容、默认搜索范围；不要求已有运行配置不可执行；原 Git mapping 原样保留，直到用户按提示在 Directory Mappings 中手动决定是否移除。 |
| GL-07 | 重新添加 | 同一目录的项目内容与 PFI 活动边界恢复，不承诺语言配置即时就绪；插件重新检查现有 mapping，不创建重复 root/module 或任何 mapping。 |
| GL-08 | 原子替换、快速连续变更与外部 model drift | 最终 authoritative Workspace Model 和 `ProjectFileIndex` 均与最新 manifest 一致，VCS 诊断来自最新可读配置；外部 `ModuleRootListener` burst 经 250 ms 防抖后强制 same-digest reconcile，ReqWS 自事件被 guard 抑制、迟到普通 event 仍按有界防抖处理为 `PROJECT_MODEL_CHANGE`；全过程无并发异常、VCS 写入、重复 roots event 或持续索引循环；已有稳定状态时瞬时 read/apply 取消不留下 `READING`/`SYNCHRONIZING` 且 `Sync Now` 可恢复，首次启动的一次取消由有界 automatic retry 自动收敛；metadata readiness 的旧 timer 不能覆盖新 generation，普通刷新不能重置 attempt 或十分钟 absolute deadline。 |
| GL-09 | manifest 损坏或临时缺失 | 保留上次有效模型，不应用部分数据，并提供稳定诊断和恢复入口。 |
| GL-10 | 路径攻击 | root mismatch、绝对 relativePath、`..` 或 symlink escape 被拒绝。 |
| GL-11 | Safe Mode | 可读诊断与 VCS 只读检查保留，受管模型更新、ReqWS roots change event 和外部进程动作禁用；恢复 trusted 后即使 digest 未变也强制重放一次完整投影，修复 blocked 期间的 Workspace Model / `ProjectFileIndex` 目录边界漂移。 |
| GL-12 | 重启恢复 | GoLand 重启后从 manifest 幂等重建 authoritative Workspace Model，通过 public `ProjectFileIndex` 验证并重新读取 VCS 配置，不依赖 Desktop 正在运行。 |
| GL-13 | 用户自定义配置 | 插件同步不删除非 ReqWS-owned module/Content Root，不创建或删除 GoLand-owned `.idea`，并且不调用 API 新增、删除、替换、重排 VCS mapping 或修改 `rootSettings`/`.idea/vcs.xml`；`.idea` 文件/symlink/identity swap、既有 ownership proof 丢失与 stable transaction 异常都保持 fail closed；GoLand 原生 metadata/save/auto-detection 的独立行为必须单独记录。 |
...
- **live 成功门禁**：authoritative Workspace Model 写入后，通过 public `ProjectFileIndex` 验证活动内容与精确 excludes；它只证明 ReqWS 管理的目录边界，不证明语言模型或运行配置已就绪。
