...
- disabled state；
- click loading/error；
- 工作区详情保持打开时，新增最后一个可选仓库后 select 与 Add 均禁用；再逻辑移除另一仓库，显示选择、按钮状态和提交 repository ID 必须同步对应新可用仓库，不得提交此前已加入的仓库；
- 仓库列表或 workspace props 更新时，仍可用的显式选择保持；已选仓库记录消失则使用当前有效项，全部可用项为空时不调用添加回调；
- zh-CN/en-US key 一致；
- screen reader label；
...
- applier 抛出的 `ProcessCanceledException` / coroutine `CancellationException` 以同一实例形成非业务 cancellation event，不发布普通 `Failed`、不恢复 clean baseline；owner scope 仍 active 时同一 worker 必须接受并成功 apply 下一 candidate，owner scope cancellation/dispose 仍关闭 coordinator；普通 observer exception 仍不能终止同步；
- observer 的 PCE/CE 各有独立回归：只在首个 `Applying` 通知抛出一次，断言同一异常实例终止 worker、applier 调用为 0、不发布 `Cancelled`/`Failed`/`Applied`，后续 offer 被拒绝；不得在每种通知中都抛出，以免掩盖 `Applying` 被错误当作可恢复 applier cancellation 的缺陷；
...
- 对没有 `.idea` 的 virgin root，首次 exact `NOFOLLOW_LINKS` attribute read 只产生内部 `PROJECT_METADATA_NOT_READY`，不创建目录、lock、state、marker 或 Workspace Model entry；同一 cold/pristine adapter 在 GoLand 创建真实 `.idea` 后可写 generation 0 并完成 target/marker 投影；
- `.idea` 曾被观察后删除、PSC mirror 含任一历史 proof、hot snapshot、file、symlink、权限/identity/canonicalization 异常都必须是 `INVALID_OWNERSHIP_STATE`/`OWNERSHIP_CONFLICT`，不得启动 readiness。preflight 后替换以及 stable handle/lock/state/temp/move/readback 的 `NoSuchFileException` 也不能按 cause chain 泛化；
...
长提示回归使用真实 Swing JToolTip 的布局、绘制和 HTMLDocument 显示内容验证宽度与文字完整性；覆盖连续长串、首尾/连续空格、HTML 字符、中文、代理对、组合重音、ZWJ emoji 和旗帜，换行不能切开字符簇。复制反馈与错误详情独立保留，新状态清除反馈。
...
- startup metadata readiness 只可由一次 production `ReqwsStartupActivity.execute → refreshOnStartup` arm；普通 `refreshAutomatically`、manual、VFS/VCS/model event 不能自行创建 lineage。deterministic waiter/probe 测试必须覆盖：missing tick 期间 full apply 始终为 1；entry 出现后恰好一个 successor 并同步；纯 timeout 仍 apply=1，使用缓存 failure + exact state-version CAS 一次性发布 `ERROR / PROJECT_MODEL_APPLY_FAILED`；后续 `Sync Now` 可恢复；
- readiness 的 newer request 继承同一 absolute deadline 与已消费 attempt，不能重新获得 10 分钟预算；旧 monitor 被 newer generation、success、hard failure、owner cancellation 或 dispose 淘汰后不能 late publish。probe 的 PCE/coroutine cancellation 保留同一实例、不启动 successor，并通过 guarded fallback 离开 `SYNCHRONIZING`；timeout/取消不得创建、删除或清空 `.idea`；
- 以注入单调时钟和 waiter/probe 覆盖 deadline 三处迟到：较新 failure 安排 monitor 时已到期，不新增 wait/probe；一次 wait 返回时到期，不执行 probe；probe 在界内开始但返回时到期，即使返回 ready 也不启动 successor。各场景只允许 exact generation/state-version/digest CAS 发布一次 terminal `ERROR / PROJECT_MODEL_APPLY_FAILED`，不得重跑 apply 或由普通 generation 重置预算；successor begin 的 latest gate 还必须重新检查 deadline；
- 以 barrier 暂停旧 read 的 begin 后续操作，再让较新 read 注册 readiness monitor，最后恢复旧 read：旧 read 不得取消或摘除较新 monitor。放行较新 monitor 并创建 `.idea` 后，它仍须自动启动一个 successor 并同步；共享 retry slot 的摘除必须在 latest-selection 边界内完成，job cancellation 留在锁外；
- 另暂停旧 cancellation completion 的 retry 安装，让较新 read 先取消并注册自己的 initial cancellation retry，再恢复旧 completion：旧 retry 不得覆盖或取消较新 slot，较新 retry 仍须恰好执行一次 successor 并自动收敛。两类恢复任务的安装与摘除都必须在 latest-selection 边界内校验 generation，安装时还须复查 exact state version，不能只在安装前检查后无条件替换 slot；
...
- 从 ReqWS 点击 GoLand；
- fixture 必须从不含 `.idea` 的 plain virgin workspace 开始；只执行一次 production startup，不点 `Sync Now`、不改 manifest、不 reopen。记录 in-memory module/content root 先于 physical `.idea` 时 Tool Window 为 `Syncing` 且没有 `OWNERSHIP_CONFLICT`，随后 GoLand 自己保存真实 `.idea` 后插件自动生成 generation-0 ownership 并继续四层收敛；
- 同一时间线记录 `.idea` 首次出现、automatic successor、authoritative/PFI/registry/PACKAGE 和 Tool Window 的时间与 `Sync Now=0`；插件进程不得调用 create/delete/forced-save `.idea`，GoLand 原生 `.gitignore`、`go.imports.xml`、module/workspace 文件须单独归因；
- 确认打开正确 workspace；
- trust flow；
- ReqWS Tool Window；
...
- virgin `.idea` readiness 的真实首开恢复必须独立于 cancellation fault injection：metadata 尚未出现时不重复 apply；出现后同 service 自动收敛。另以隔离 fixture 验证十分钟 timeout/后续 `Sync Now`、mid-session 删除和 symlink/file replacement；后两者必须立即 hard fail，不得重新进入 startup wait。
...
- 保存可正常解码和展示的真实 GoLand JPEG 或 PNG 截图到需求包 `ui/` 目录，并在视觉设计文档中与原型同屏对照；候选截图不得写成 `PASS` 或 `GO`；
...
- 在 GoLand 创建 `.idea` 时预置/观察平台 sentinel 文件；readiness 成功、timeout、disable/enable、close/exit 前后证明 ReqWS 未创建、删除、清空或强制保存该目录，只在完整 stable-directory 门禁通过后管理自己的 `reqws-managed-project-model.json`；互斥绑定真实目录 inode，legacy lock 子文件不是写权限依据；
...
| S-17 | virgin `.idea` 尚未创建、随后为 real directory/file/symlink，或观察后被删除/替换 | 只有 startup + cold + pristine + never-observed 的 exact missing 可等待；real directory 只唤醒完整重验，其他 entry/删除/替换 hard fail，外部目录零写入，插件零 `.idea` create/delete。 |
...
包含[可选同步追踪](../technical-design.md#182-可选同步追踪)的候选在本节验收前，以 `-Dreqws.sync.trace=true` 启动新的 GoLand JVM，记录 exact source/ZIP/installed JAR、VM options、run/JVM 和 project-service trace identity；操作步骤见[开发指南](../../../guides/development-guide.md#goland-同步追踪)。默认关闭的旧候选没有追踪行，不能据此倒推其事件数为零。开启追踪是测量条件，不代替任何四层或安全门禁。

| 测量项 | 计数与耗时来源 |
|---|---|
| refresh cadence 与范围 | 实际 watcher tick 的去重记录和单调时间差；结合生产调用点/定向刷新范围核对只有 fixed manifest 与直接父目录。名义 1,000 ms 不能代替实际 tick 或被采样遗漏的时间。 |
| VFS→同步 | received event/batch、matching event/batch 和防抖后 shared-sync dispatch 分列；保留计数单位，不能把 batch 数、event 数和 apply 数混为一项。 |
| roots→重放 | 按事件类型、Workspace Model-only 标志、guard/拒绝原因和实际防抖 trigger 分组；分别计算 `PROJECT_MODEL_CHANGE` 与 follow-up，核对 guarded self-event 没有产生 drift replay。 |
| coordinator | 以 attempt/source 关联 submit、实际 apply、clean no-op 和终态；latest-wins 合并前的提交数不等于 apply 数，elapsed 来自相应实际开始/结束边界。 |
| 四个 adapter 阶段 | authoritative model、PFI、registry、只读 VCS 的真实 span 与结果；未进入的阶段标为未执行，begin 缺少终态时标为不完整，不能算成功或零耗时。 |
| registry | 每次 invocation 单列 read、wait、ordinary notification 和 elapsed，包括首次读取成功的零等待/零通知路径；通知尝试、门禁拒绝与实际发布分开，仍按既有至多一次和 30 秒终止条件判定。 |
| 原生开销与文件保护 | CPU、线程、内存、EDT/indexing、GoLand 原生进程和下载来自各自采样/日志；manifest、`.run`、源码和 Go module 文件仍使用明确的前后快照，不因 trace 存在就宣称持续 WATCH 或全事件零写入。 |

同一原生 log 在 collector 续接时可能重放前缀。按原 log identity/byte range 去重，并以 run/JVM、service identity、event sequence 关联记录；collector 的 `observedAt` 不作为事件重新发生的时间。每组数据记录覆盖区间、原日志前缀/哈希、seq 连续性与轮转情况；缺段、截断、sink 失败或采集结束后的区间记为不可判定，不能用空记录证明零事件。不同 JVM 的单调时间不直接相减。

用于验收前先验证测量设施自身：关闭时无记录且不调用 formatter/sink；开启时格式只含固定枚举与数值；sink 抛错不改变 apply/no-op/digest 或真实取消/observer 的传播；registry 快速成功路径也完整计数。测量开关打开和关闭的性能观察分别标记。上述方法只补证据获取，50+20、10 次变更、100 次 rapid rewrite、idle/no-loop、四层一致与本节失败条件保持不变。
...
添加/逻辑移除回归须保持同一详情抽屉：初始 A/B → 添加 C → 确认没有可选项时 Add 禁用 → 移除 B → 不重新打开详情或重新选择，直接添加界面所示 B。实际提交目标、完成后的 manifest 与 `.code-workspace` 必须一致，B 原目录保留；不能用关闭重开抽屉规避陈旧选择状态。失败时分别留存提交前选择与实际错误目标，不把非原子采集的 AX 和截图视作同一瞬间。

工作区文件恢复回归先将已有 `.code-workspace` 原字节保留到测试备份，再用实际 Refresh 确认 Missing 与编辑器入口禁用；磁盘 state 的既有 ready 不代替实时文件诊断。通过详情中的 Regenerate workspace file 恢复 Ready，并检查新文件的 folders 对应当前活动集、manifest 与仓库源码保持、备份未被移回以伪造生成；记录文件内容与 inode，区分正确重建和原文件复位。
