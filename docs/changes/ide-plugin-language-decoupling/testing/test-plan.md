...
S1 已提交；其阶段必要回归证据见 [S1 实施记录](../tasks/s1-core-sync-decoupling.md#8-本轮实施记录2026-09-18)。S2 核心代码已落实，当前输入与必要回归证据见 [S2 实施记录](../tasks/s2-scheduling-dependency-cleanup.md#8-本轮实施记录2026-09-18)，V 尚未执行。目标契约见[技术方案](../technical-design.md)，边界见[开发与测试规范](../../../standards/ide-plugin-development-testing.md)。本文件维护共用 fixture、覆盖目标与选例规则；阶段执行分别在 S1/S2 和 V 独立文档维护。新计划替代旧 GoLand 常规测试矩阵中的语言门禁和重复人工排查；其通用安全/并发自动化回归仍需保留，不以新表行数替代旧测试覆盖。
...
当前阶段为 S2；必要回归按任务内实施记录据实更新，不将 S1、历史 PR 或旧工件结果继承为当前输入的证据。V 尚未执行，完整验收结论仍待最终组合代码验证。
