...
这是实施前方案，不是已完成变更记录。本次仅更新本地文档，不改插件代码或自动化测试，不升级任何版本，不变更 CI/Release，也不安装、重启或发布插件。
...
| 当前说明与旧用例清理 | 各任务随实际改动维护，S2 收尾 | 通用回归保留；历史 verification、输入 digest 和冻结参考不改写。 |
...
本次本地文档变更只完成规范、方案及独立任务整理，不满足上述实现完成标准，也不产生新的功能 GO。
