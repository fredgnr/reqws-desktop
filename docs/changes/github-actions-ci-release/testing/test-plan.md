...
不验证未实现的 Developer ID、公证、Gatekeeper 跨机器接受、DMG、自动更新或 Marketplace 发布。不读取自定义 secrets，不在未经明确授权时创建、移动或删除远端 tag，也不安装/重启真实 GoLand。
...
| 资产集合 | 下载的文件必须恰好为 app/plugin ZIP 及其 checksum；缺失、额外、篡改、校验和不匹配均失败。 |
...
首次受控 tag 发布后，从 GitHub 下载三份公开资产，而非复用 runner 工作目录，并记录：

- 两份 ZIP 的 SHA-256、大小、精确名称，确认不存在 x64 app 附件。
...
- Release 已从 draft 公开，资产集合完整且非空，页面保留 ad-hoc/未公证和 unsigned 插件限制。
