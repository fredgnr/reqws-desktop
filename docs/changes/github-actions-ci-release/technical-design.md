...
`buildPlugin` 依赖生产禁用 API 和 descriptor 策略检查。基线 SDK 为 GO 2026.2，descriptor 只有 `since-build="262"`。CI 从 `package.json` 读取版本，保留编译和全部单元/Light/Heavy 平台覆盖，构建后经 Release ZIP 校验路径导出 `plugin.zip` 与 `version.txt`。独立 job 冻结官方 API 集合，`goland-verification.yml` 对同一 ZIP 执行 API 矩阵，不设置 `max-parallel`，由 GitHub 按可用 runner 和账户配额调度并发。完整 GoLand Starter/Driver 已移到本机，任何 CI 工作流都不启动它或请求 IDE 授权。影响分类仅提示本机集成建议，不触发用户电脑。首次迁移未验收前 PR 保守使用完整正式集合，不能提前退为抽样。
...
若运行中的 job 日志下载返回 404，应先看 Actions 实时步骤输出；日志归档尚不可用本身不是卡死证据。下载/解压和 Verifier 活跃计算从 info 输出、日志增长及进程/线程样本判断，缺终态仍不能通过。日志诊断不改变最低 262、固定 UI 代表版本、冻结目标清单或候选 ZIP 一致性约束，不启动完整 IDE。
