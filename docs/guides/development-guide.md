...
生产代码使用公开 261 API：保留 GoLand 既有 workspace-root Content Root；每个插件创建的 target exclude 都配一个虚拟 companion marker exclude。Project Model ownership 的权威文件是 `<workspace-root>/.idea/reqws-managed-project-model.json`：每次 mutation 前 verified atomic 写入 managed + recovery claims，同一 JVM 不清 recovery，legacy PSC 只作一次迁移；进程重启后的 cold service 若 target+marker pair 仍完整就保留 recovery 并完成精确删除，只有两者都不存在时才压缩 recovery，partial proof 必须冲突。VCS mapping 区分 `CREATED` 与 `BORROWED`，权威文件 `<workspace-root>/.idea/reqws-vcs-ownership.json` 使用 v2 pending add/remove；实际 plugin add/删除前先落盘，且 pending 必须按 actual live → expected whole-list 的真实变化生成，不能只看 planner additions/removals；任何 cold pending 都不能授权删除，本轮观察到的 external event、snapshot mismatch 或 retry 都要在继续前把 `CREATED` 持久降为 `BORROWED`。VCS legacy PSC v1 同样只在 atomic 文件不存在时迁移；任一 atomic 文件存在但损坏、版本不支持或 identity 不匹配时都禁止 fallback PSC。expected/set/self-event/quiescence 统一使用平台 observable canonical form：exact directory 最后项胜出并保留完整对象，再按 directory 排序。后台 plan 绑定 mapping change revision/完整 snapshot，但 pending 只代表已发布事件；不同 directory 的 live-only 项必须连同 `rootSettings` 保留，pending-only 或同 directory 不等对象均等待事件，达到上限后零写入 fail closed，绝不自动恢复 pending-only。只有当前 plugin write 的同步回调是 self-event；延迟的同列表回调仍为 external。Settings writer 由 EDT final equality/set 串行，不走 EDT 的 pooled auto-detect writer 由公开 `VCS_CONFIGURATION_CHANGED` snapshot capture 与有界 quiescent merge-retry 覆盖；最终 verify 后到达的 external event 使 digest baseline dirty并强制下一次 automatic reconcile。自动化必须把 pooled writer 的 live mutation 与 event publication 拆开，并用独立线程证明不静默丢 mapping 或 `rootSettings`；public API 没有 CAS，事件本身也不跨重启持久化，因此不宣称平台级线性化；未能判明的当前状态必须零写入。Manual intent 跨 automatic candidate/read failure 保留到下一份有效 candidate 开始 reconcile；project service 的 terminal dispose probe 贯穿模型、VCS、refresh 与 digest gate。Safe Mode 只通过稳定 `TrustedProjects.isProjectTrusted` 查询，并仅在 blocked 期间低频检查 trust transition；禁止为方便改用 `@Internal`、`@Experimental`、反射或私有 API。
