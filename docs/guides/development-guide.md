...
测量后移除该选项或设为 false，并按授权流程核对新会话；不清空旧日志制造零记录。追踪仍只含固定枚举和数字，不输出 workspace 路径、digest、URL 或异常文本，不新增网络/IPC 导出，也不改变同步判定。后续实现删除 Go registry 专用观测，但保留必要通用追踪；本次文档提交未改变 trace 代码或 schema。
