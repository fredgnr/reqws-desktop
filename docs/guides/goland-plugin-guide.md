...
| `Syncing… / 正在同步…` | 正在收敛 authoritative Workspace Model、live `ProjectFileIndex`、Go Modules registry，并读取当前 VCS 配置。全新 workspace 尚无 `.idea` 时也会保持此状态，等待 GoLand 自己保存 metadata；这不是 `OWNERSHIP_CONFLICT`。 | 等待完成，不要用重复点击制造无意义重试；virgin metadata 等待最长 10 分钟。 |
...
若 GoLand 在中国区首次启用 Go Modules 时提示配置 GOPROXY，或日志出现 `GOPROXY_MISSING`，先通过 GoLand/Go 的原生设置配置组织批准的代理，再重开项目或选择 `Sync Now`。ReqWS 不读取、写入或绕过 GOPROXY；`GOPROXY=off` 只适合没有外部依赖的隔离验证，不应作为日常项目默认值。
