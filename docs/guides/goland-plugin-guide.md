...
- 不读取 Go Modules registry、等待语言服务或调用 Go command/进程 API；正常项目模型更新仍可能触发 IDE 原生语言分析、进程或网络行为，这些由 IDE 和用户环境决定；
...
`check:goland` 会运行插件测试、项目/结构检查和配置的 GoLand 2026.1.3/2026.2 Plugin Verifier。成功构建只证明产物可生成；它不能代替最终候选所需的真实 GoLand 仓库视图、目录范围、自动刷新和生命周期验收。
...
GoLand 在 Safe Mode 打开项目时可能暂不加载原项目配置；此行为已在 2026.1.3 和 2026.2.1.1 中观察到。若 IDE 提示信任后需要重新打开，先保存需要保留的编辑，再通过 **File → Close Project** 关闭项目，并从欢迎页重新打开同一个 workspace root。ReqWS 会随正常项目模型加载自动重新同步。此时直接信任后出现的 `OWNERSHIP_CONFLICT` 可能与尚未替换的临时项目模型有关；先按原生提示重开，仍失败时按第 12 节保存诊断排查，不要删除 ownership 或清空 `.idea`。

全新 workspace 第一次打开时，GoLand 可能先建立内存项目模型，稍后才把 `.idea` 保存到磁盘。此时 ReqWS 会保持 **Syncing… / 正在同步…**，每秒只检查一次固定 `.idea` entry，并在 GoLand 创建真实目录后自动重读最新 manifest；不需要点击 `Sync Now`、修改 manifest 或重开项目。等待从首次遇到该状态起最多 10 分钟；若仍未创建，插件进入 `Error / PROJECT_MODEL_APPLY_FAILED`，此后可以在 GoLand 项目 metadata 就绪后选择 `Sync Now`。

ReqWS 不会创建、删除、清空或强制保存 `.idea`。不要为了加快首次同步而手工创建/清空该目录；若 `.idea` 是文件、符号链接、身份已变化，或既有 ownership state 丢失，插件仍会按真正的所有权错误停止，而不会把它当成首次等待。
...
![ReqWS Tool Window 同步态实现候选](../changes/goland-plugin-support/ui/tool-window-synchronized-light.jpg)

[打开原尺寸截图](../changes/goland-plugin-support/ui/tool-window-synchronized-light.jpg)。该图用于定位面板区块；具体候选、实测范围与未完成验收见[修复报告](../changes/goland-plugin-support/testing/verification-2026-09-13.md)，截图不代表完整 GUI 验收通过。
...
| 状态 | 含义 | 建议操作 |
|---|---|---|
| `Active / 活动` | manifest 声明该仓库、目录是普通 Git repository，live `ProjectFileIndex` 已纳入该目录，且当前有一个等价的 `Git` mapping。它不证明语言模块或运行配置就绪，也不证明远端、工作树或分支健康。 | 通常无需操作；结合顶部状态判断是否还存在 retained/extra mapping。 |
...
跳过 VFS 文件事件的防抖等待，重新读取固定 manifest，重放 Project Model 与 live `ProjectFileIndex` reconcile，并重新读取当前 VCS Directory Mappings。它进入与自动同步相同的串行、latest-wins 协调器，但不会自动修复 Git Roots。
...
同步保留正常项目模型更新与目录范围检查，不再额外通知或轮询 Go registry。`Sync Now` 不等待语言服务，也不能用来保证 Go package 或运行配置立即就绪。Safe Mode 不修改项目模型。
...
该操作只定位并打开文件，不创建、修复或写回内容。打开的是普通 GoLand 编辑器，插件不会将文件设置为只读。ReqWS Desktop 是唯一 writer；排障时可以检查字段和错误位置，但不要把手工编辑 manifest 当作日常仓库增删流程。
...
| `Syncing… / 正在同步…` | 正在收敛 authoritative Workspace Model 与 live `ProjectFileIndex`，并读取当前 VCS 配置。全新 workspace 尚无 `.idea` 时也会保持此状态，等待 GoLand 自己保存 metadata；这不是 `OWNERSHIP_CONFLICT`。 | 等待完成，不要用重复点击制造无意义重试；virgin metadata 等待最长 10 分钟。 |
| `Synced / 已同步` | authoritative Workspace Model 与 live `ProjectFileIndex` 的目录范围均已收敛，当前只读检查也未发现 Git Root 差异，摘要已推进；不表示语言分析、构建或运行配置就绪。 | 无需操作。 |
| `Partially Available / 部分可用` | 至少一部分安全投影可用，但存在目录缺失、Git Root 待手动配置、Project Model ownership 冲突，或 live `ProjectFileIndex` 未收敛。 | 查看仓库行和底部提示；VCS 差异按第 9 节手动处理；`PROJECT_CONTENT_NOT_CONVERGED` 按第 12 节处理。 |
...
4. 插件确认 live `ProjectFileIndex` 的目录范围收敛后，新仓库在 Repositories 卡片中显示为 `Active`；若缺少 Git Root，会显示手动 Directory Mappings 提示。
...
5. live `ProjectFileIndex` 不再纳入保留目录；插件不会删除目录，也不保证 IDE 语言模型或既有运行配置立即变化。

### 重新添加与冷启动

重新添加同一仓库后，插件恢复 Project Model 与 live `ProjectFileIndex` 目录范围并重新检查现有 mapping；它不创建重复 root/module，也不创建 mapping。若 Git Root 先前已由用户移除，按第 9 节手动恢复。GoLand 重启时会重新读取 manifest 和 VCS 配置；不要求 ReqWS Desktop 同时运行，也不会仅凭持久摘要跳过重建。

### 语言服务与运行配置

Go package 发现、SDK、依赖解析以及 run/test/debug configuration 由 GoLand、语言插件和用户配置负责。仓库有无 `go.mod`、其内容或位置不会改变 ReqWS 的活动成员及同步判定。ReqWS 显示 `Synced` 后，语言模型或运行配置仍可能尚未就绪；删除旧 Go 补偿后，语言配置刷新也可能延迟。遇到语言错误时通过 IDE 原生设置和日志排查，不能把 `Sync Now` 视为语言配置修复操作。

逻辑移除只改变 ReqWS 管理的项目内容范围，不禁止执行磁盘上保留的代码；重新加入也不承诺既有运行配置立即恢复。
...
底部摘要可能显示 `REPOSITORY_MISSING`、Git Root missing/wrong-VCS/retained 提示、`OWNERSHIP_CONFLICT` 或 `PROJECT_CONTENT_NOT_CONVERGED`。最后一项表示 live `ProjectFileIndex` 的目录范围未符合 manifest；Tool Window 显示统一稳定错误码和“项目文件索引未收敛”，present repository 因而不得显示 `Active`。插件会保留能够安全应用的项目模型以及全部 VCS 配置，不会为追求“全绿”覆盖用户 mapping。目录问题回到 Desktop 修复；VCS 差异按第 9 节在 Directory Mappings 中手动处理；project content 未收敛时先等待一次自动重放，必要时选择 `Sync Now`，持续失败则复制诊断并检查 GoLand 日志。
...
- 首次 metadata 等待超时也使用 `PROJECT_MODEL_APPLY_FAILED`。先确认 GoLand 已自行创建真实 `<workspace-root>/.idea`，再选择 `Sync Now`；不要手工创建、删除、替换或清空 `.idea`。若诊断为 `OWNERSHIP_CONFLICT`，按真实 ownership/路径异常处理，它不是 readiness timeout。
...
- 同步态截图用于界面导览；实际长提示、VoiceOver 和完整规模验收的限制以修复报告为准。

进一步资料：

- [ReqWS 使用说明](user-guide.md)：Desktop 的仓库、工作区、设置、恢复和数据保护。
- [ReqWS 开发指南](development-guide.md)：插件构建、测试、兼容与开发约束。
- [IDE 插件语言解耦](../changes/ide-plugin-language-decoupling/README.md)：当前同步语义、S1/S2 进展及最终验收计划。
- [GoLand 插件支持需求包](../changes/goland-plugin-support/README.md)：通用 manifest、项目模型、VCS、安全契约和旧候选验证记录。
